...
- **每天第一張交通卡要從飯店算起**（2026-09-24）：京都的飯店在八条口南邊，走到八条東口約 13 分、到北側的巴士總站或地下鐵約 20 分。
  Day 7 曾經寫「08:30 京都車站 → 清水寺」，實際要 08:00 從飯店出發
- **趕預約的交通要寫出「一定要搭的那一班」**：例如 Day 3 姬路 → 三ノ宮只有 13:12 的新快速來得及 14:00 的 Mouriya，下一班會遲到
...
航班是**台灣虎航**（票種 tigersmart）：去程 IT212 9/26 15:15 桃園第一航廈 → 19:00 關西第 1 航廈、回程 IT711 10/3 12:20 → 14:15（2026-09-24 由原本誤寫的中華航空 CI158／159 更正；虎航報到櫃檯起飛前 2.5 小時開、45 分鐘前截止）。
...
- **記憶縮放**（2026-09-24 使用者要求：原本每換一個彈性方案就被重置成整張地圖，沒辦法細看）：
  - **只有換天（或第一次載入）才會重新定位**；換方案、開關類別、時間、路線、推薦景點、其他方案，一律保留目前的縮放與位置
  - 換方案時新的點不在畫面裡，只**平移**過去（`panInside`），不改縮放；從彈窗換的會在新的點上再打開彈窗
  - 每一天看到的位置存在 sessionStorage `mapViews`：換到別天再換回來、從行程表「看這天的行程」連回來，都停在剛才的地方；關掉分頁才清掉
  - **「⤢ 回到全觀」**浮動按鈕在左上角縮放鈕下面：這一天全部地點與路線都在畫面裡時隱藏，放大或移開才出現。網頁版與手機版同一套
  - 觸控裝置（`@media (pointer: coarse)`）把方案按鈕放大到 34px、面板與「回到全觀」按鈕加大內距，手指才點得準
...
- **依交通方式的實際路線**（2026-09-21 起 Day 1、3、8；**2026-09-24 起八天全部**）：
  使用者要求「不能再有隨意連線，一定要考慮人是用什麼方式過去的」—— Day 4 從飯店到第一個景點曾經直接連一條直線
  - 鐵路、地鐵、市巴士沿著 **OpenStreetMap 上該路線的 route relation** 畫（軌道；公車是道路），
    在上下車站之間找最短路；步行由 **OSRM** 計算；起訖點與車站之間、轉乘的兩站之間自動補步行
  - 定義在 `data/routes.json`，**照行程表的交通卡寫**，由 `scripts/build-routes.js` 產生
    `docs/map/routes.json`（**不要手動編輯**）。`build-places.js` 跑完會自動接著跑它
  - `data/routes.json` 的區段用**行程表項目的時間**當 key（`"from": "11:00", "to": "13:20"`），當天從飯店出發是 `"start"`；
    `legs` 是預設走法，彈性方案走法不同時寫 `cases`（`{ "from": ["B"], "to": ["E"], "legs": [...] }`，由上往下第一個符合的生效）
  - `build-routes.js` 會把**前後兩站的每一組方案**都算一次（Day 2 就有 78 段），所以地圖上不管選哪個方案都有實際路線；
    同一個項目裡的好幾個地點（奈良公園 → 東大寺 → 春日大社）之間一律步行；
    彈性行程選了沒有地點的方案（Day 2 早餐 D 木津市場）時，需要「跨過它」的區段（Day 2 的 `start → 11:00`）
  - **八天都要定義、每一段都要定義**，少一段 `build-routes.js` 會直接報錯並列出缺哪一段；
    行程改了時間，沒用到的區段與 case 會印出警告；步行超過 2.6 km 也會提醒（可能少寫了搭車）
  - 地圖樣式（使用者要求：走路虛線、搭車實線、不同交通工具不同顏色）：
    **鐵路（JR・私鐵・嵐電）＝主色實線、地鐵＝次色實線、市巴士＝綠色實線（`--c-bus`）、步行＝灰色虛線**。
    搭車的線底下墊白色描邊；上下車站畫白底小圓圈（放大到 12 級以上才顯示）；
    每段線上疊一條看不見的 20px 粗線，**滑鼠滑過或手指點一下**都會顯示「搭哪一條線、哪站到哪站」
  - 圖例在**地圖左下角**，只列當天用到的交通方式（原本在狀態列）。萬一缺路線資料會畫紅色虛線並在狀態列寫出段數 —— 正常應該是 0
  - 步行路徑若比直線遠 3 倍以上（例如關西機場航廈與車站之間的室內連通道 OSM 沒畫到，
    OSRM 會繞出 3.4 km），直接畫直線
  - relation 先用 **OSM 主 API**（`/api/0.6/relation/<id>/full`）抓，失敗才用 Overpass（主站常回 429／504）；
    抓過的 relation 與算過的步行路徑都存在 `scripts/.cache/`（不進版控），重跑不會再打 API
  - OSM 上**京阪本線的班次 relation 已被刪除**（2026-09-24，HTTP 410），改用鐵道線路 relation 1921059；
    這種 relation 的車站沒有角色，腳本會把有名字的 node 成員都當成車站
  - 市巴士用京都市交通局的路線 relation：205 號（反時計回り 13361782 北大路BT → 金閣寺道）、102 號（3649326）、
    206 號（反時計回り 13363356 京都駅前 → 五条坂；時計回り 13363355 清水道 → 京都駅前）、4 號（12916334 → 京都駅前）；
    公車站在 OSM 多半是 platform 角色
...
- **門牌級座標 `FIXED_COORDS`**（2026-09-23 起）：Nominatim 對日本地址只到「丁目」中心。需要精確位置的地點
  （大阪與京都的飯店、Day 6 晚餐 Conel）用國土地理院「地址檢索」（`https://msearch.gsi.go.jp/address-search/AddressSearch?q=地址`）取座標，
  寫進 `FIXED_COORDS`，優先於 `QUERY_OVERRIDE` 與快取。飯店的丁目中心和門牌差約 160m，到新今宮站是 6 分鐘不是 4 分鐘
  - 「奈良公園」也在 `FIXED_COORDS`：查到的是公園面積中心（東大寺東邊），地圖會變成近鐵奈良 → 奈良公園 → 走回東大寺的折返；
    行程是「從近鐵奈良站往東走到東大寺，沿路就是奈良公園」，改標在這條路上（登大路的奈良公園觀光導覽圖）
- **序號**：當天最早的點若是機場，或前一晚住的飯店（見下一項），編號從 0 起算（代表抵達／出發點），其餘從 1 開始。
  注意 `makeIcon()` 不可用 `seq ? ...` 判斷，因為 0 在 JS 是 falsy
- **每天從飯店出發**（2026-09-24 使用者要求）：`build-places.js` 依「入住｜」卡片記住前一晚的飯店，
  Day 2 起每天自動加一個 `start: true` 的出發點（標 0）。Day 4 是移動日：0＝剛退房的大阪飯店，當天入住的京都飯店照時間編號。
  當天第一張卡片是從飯店出發的交通或退房時，時間標籤寫「09:00 出發」，否則只寫「出發」。
  有實際路線的天數（`data/routes.json`）要記得定義「飯店 → 第一站」這一段。「全部」檢視不標出發點（就是前一天的住宿）
- **編號與路線不受類別篩選影響**：用當天全部地點排序；關掉某類標記，其他點的號碼不變
- **同一個地點不要連續出現兩次**：交通卡的地圖連結要指「起點」（例如「步行｜JR 姬路站 → 姬路城」指姬路站），
  不要和下一張景點卡指同一處 —— Day 3 曾因此標記 2、3 疊在一起，看起來從 1 直接跳號
- **彈性行程在地圖上可以切換方案**（2026-09-24 起）：
  - `places.json` 的 `places` 只含方案 A；`slots` 是每組彈性行程的全部方案（A～E 的名稱、圖示、地點）。
    方案 A 的地點取卡片本身的 `.map-link`，B～E 取方案小卡的 `.pr-map`
  - 方案地點的座標在 **`data/plan-coords.json`**（店家取食べログ地圖頁的 `center=`，不要用 Nominatim 查店名；
    景點用 Nominatim 查過再核對）。新增方案時要補上，沒補的 `build-places.js` 會退回 Nominatim 並印警告
  - 地圖右上角「🔀 彈性方案」面板**預設收合**；「📍 其他方案的位置」**預設隱藏**（虛線小圓點＋字母）。
    這兩個是使用者要求的「不要太亂、要能收起」，不要改成預設展開
  - 選擇存在 localStorage `mapPlanSel`；地圖只標目前選的方案，時間標籤寫「19:20・A」
  - 路線逐段查表：相鄰兩點在 `routes.json` 找對應的區段照交通方式畫；每一組方案都事先算好，換方案也不會出現直線
  - **每組彈性行程的方案 A 都要有 `.map-link`**，否則地圖上那個時段沒有點（Day 1 22:00「在飯店休息」例外）。
    曾經 Day 4～7 的晚餐都沒有地圖連結，地圖上看不到
...
- **三種項目分類**（2026-09-21 起）：每個 `.timeline-item` 都必須有 `.tl-kind` 標示
  - `.is-main` ＋ `主行程`：航班、住宿 check-in/out、5 個已預約項目、報到與退房，
    ...
    Day 6 瑞饋祭的神轎出遊，以及 Day 5 19:10 的**抹茶蛋捲**（使用者指定一定要買到的伴手禮，
    放在彈性方案裡會因為換方案而漏掉）。左側為實線邊
  - `.is-move` ＋ `交通`：**所有移動與過關**。交通不是「行程」而是兩個行程之間的過程，
    所以獨立成一類，左側為灰色點線、標籤是灰字外框，讓視線可以直接跳過
  - `.is-flex` ＋ `彈性`：其餘餐飲、購物、機動景點。左側為虛線邊，內含方案輪播
  - 目前為 **21 主行程 / 25 交通 / 19 彈性**（共 65），**不應有未標示的項目**
  - 交通項目**不再另外掛 `.tl-tag.transport`「交通」pill**，那會和分類標籤重複顯示同一個字
- **方案輪播 `.plans`**：方案 A 一律是原訂行程，B～E 為替代方案
  （2026-09-24：19 組、共 59 個替代方案 —— B、C 各 19，D 18，E 3）
  - 例外（2026-09-24）：**Day 2 09:00 早餐**原訂的木津市場 9/27 週日公休，方案 A 改成千成屋珈琲，木津市場移到 D 留作參考；
    **Day 7 18:30 晚餐**的方案 A 是使用者指定的**博多かわ屋 京都烏丸四条店**（就在回飯店的地下鐵四條站旁），原本的 A 往後順移
...
- **導遊型交通指引**（2026-09-22 起，原名「step-by-step 交通指引」，使用者要求改名）：交通卡片裡「導遊型交通指引：」＋ `<ol>`
  ...
  - 目前（2026-09-24）56 個方案有指引；沒寫的替代方案：Day 2 18:45 晚餐 B～D、Day 4 20:45 晚餐 D、Day 5 19:20 晚餐 D（依規則），
    Day 2 早餐 D 木津市場（當天公休）、Day 7 晚餐 B「四條／河原町一帶」（就是 15:30 採購的同一帶）
...
- `audit/records/2026-09-24.md` - 最新審核記錄（航班改台灣虎航、抹茶蛋捲、博多かわ屋、地圖從飯店出發與彈性方案切換、八天時間安排掃描）
- `audit/records/2026-09-23.md` - 審核記錄（彈性方案 B～E 的導遊型交通指引、飯店門牌座標、大景點照片與地圖導覽、新今宮出發）
...
- `docs/selector/foods.html` - 美食選擇器（橙色主題，121 筆）
...
- `reference/update-2026-09-24/` - 航班、抹茶蛋捲（祇園辻利）、博多かわ屋、任天堂博物館營業時間等查證來源；虎航官網是動態網頁、airpaz 擋爬蟲，記在 `查證筆記_航班與無法備份的頁面.md`
...
- `scripts/build-routes.js` - 依 `data/routes.json` 產生地圖上依交通方式的實際路線（八天、每一組彈性方案；見上方「docs/map/ 行程地圖」）
