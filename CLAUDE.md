...
- **門牌級座標 `FIXED_COORDS`**（2026-09-23 起）：Nominatim 對日本地址只到「丁目」中心。需要精確位置的地點
  （目前是大阪的飯店）用國土地理院「地址檢索」（`https://msearch.gsi.go.jp/address-search/AddressSearch?q=地址`）取座標，
  寫進 `FIXED_COORDS`，優先於 `QUERY_OVERRIDE` 與快取。飯店的丁目中心和門牌差約 160m，到新今宮站是 6 分鐘不是 4 分鐘
...
- **Day 6（10/1）瑞饋祭優先＝13:00 神轎出遊**（2026-09-22 使用者要求）：
  08:05 出發 → 09:00 金閣寺（早上整段，約 2 小時）→ 11:30 北野午餐 → 12:40 神轎出遊（13:00 出發）→
  14:45 嵐山 3 小時、渡月橋看日落（17:42）→ 17:50 阪急嵐山 → 河原町 → 18:30 木屋町晚餐 → 20:00 夜間
  - **不排 09:00 出御祭**：排進去會變成 北野 → 金閣寺 → 回北野 的折返（見「行程動線原則」）
  - **御旅所（ずいき神轎＋攤販）是 20:00 夜間的方案 E**，不排在主時間軸上：排在嵐山之後會把晚餐擠到 19:30、
    鴨川擠到 21:00（使用者反映鴨川時段被壓縮）。ずいき神轎 10/1～10/4 上午都在御旅所、攤販開到 22 點左右，
    晚上去一樣看得到；御旅所周邊晚上沒有其他景點，所以只值得當成選項
  - **北野天滿宮本社沒有攤販**，攤販只在西ノ京的御旅所（地圖用 OSM 的「北野神社御旅所」，
    Nominatim 查「北野天満宮御旅所」會落到中京區壬生的另一個點）
  - 16:00 御旅所的着御祭・八乙女舞和嵐山夕陽衝突，行程選了嵐山，卡片裡有寫明
- **導遊型交通指引**（2026-09-22 起，原名「step-by-step 交通指引」，使用者要求改名）：交通卡片裡「導遊型交通指引：」＋ `<ol>`
  - **彈性方案也要有自己的指引**：`.plan-body` 裡的 `.plan-route`（`.plan-route-title`「🧭 導遊型交通指引」＋ `<ol>`，
    只有一步時用 `<p class="plan-route-step">`，補充的另一種走法放 `.plan-route-note`），寫「從前一站怎麼到這個方案」。
    有前一張交通卡時寫成「照上一張交通卡到〇〇站，往〇步行約 N 分鐘」；更好的走法不經過那張卡時寫「不走上一張交通卡：…」
  - 目前（2026-09-23）55 個方案有指引；依規則不寫的 5 個：Day 2 18:45 晚餐 B～D、Day 4 20:45 晚餐 D、Day 5 18:45 晚餐 D
  - **可以不寫的情況**（使用者訂的規則）：前一張交通卡已經寫過同樣的走法；或這個方案離方案 A 走路 3 分鐘以內
...
- `audit/records/2026-09-23.md` - 最新審核記錄（彈性方案 B～E 的導遊型交通指引、飯店門牌座標、大景點照片與地圖導覽）
- `audit/records/2026-09-22.md` - 審核記錄（主行程計畫B、換天捲動位置、大丸梅田店改名 LUCUA SOUTH、麵包類擴充、
  Day 4 空檔與 teamLab、Day 6 瑞饋祭規劃與動線原則、Day 6 鴨川時段與御旅所改夜間方案、Day 3 神戶晚餐、導遊型交通指引）
...
- `docs/selector/foods.html` - 美食選擇器（橙色主題，119 筆）
...
- `price` 用字串比對判斷價位：含「¥1,000」就算中價位，所以千円以內要寫成「¥999 內」才會是平價。高價位的比對字串要涵蓋新價位 —— 2026-09-22 補上「¥6,000」「¥7,000」，原本「¥6,000-7,999」一個都比不到、被判成平價
